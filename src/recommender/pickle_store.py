...
DEFAULT_EMBEDDING_PATH = PROJECT_ROOT / "data" / "processed" / "embeddings.pkl"


class PickleEmbeddingStore(EmbeddingStore):
    """Pickle-backed storage implementation for vector embeddings."""

    def __init__(self, path: str | Path = DEFAULT_EMBEDDING_PATH):
        self.path = Path(path)
        ...
        if self.path.exists():
            ...
            self.fic_ids = np.array([], dtype=np.int64)
            self.embeddings = np.empty((0, 0), dtype=np.float32)

    def _load(self) -> None:
        """Loads embedding metadata and arrays from disk."""
    ...
    def _save(self) -> None:
        """Serializes current state to the configured pickle file path."""
    ...
    def get_all_embeddings(self) -> tuple[np.ndarray, np.ndarray]:
        """Returns tuple of all fic IDs and embedding matrix."""
        return self.fic_ids, self.embeddings

    def save_embedding(self, fic_id: int, embedding: np.ndarray, model_name: str) -> None:
        """Save or update a single fic embedding vector."""
        ...
        if model_name != self.model_name:
            raise ValueError(f"Expected model {self.model_name} but received {model_name}")
        ...
        if len(matches) > 0:
            ...
            self.fic_ids = np.append(self.fic_ids, fic_id)
    ...
    def save_embeddings(self, fic_ids: np.ndarray, embeddings: np.ndarray, model_name: str) -> None:
        """Save or update many embeddings at once."""
